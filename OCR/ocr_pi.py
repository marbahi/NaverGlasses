# multiocr.py - Raspberry Pi (rpicam/Picamera2) ready
# Prasyarat:
#   sudo apt update && sudo apt install -y python3-picamera2 rpicam-apps
#   (opsional) sudo apt install -y libatlas-base-dev
#   pip install easyocr pygame gTTS opencv-python numpy
...
class PiCamCapture:
    """Adapter sederhana agar Picamera2 memiliki API mirip cv2.VideoCapture."""
    def __init__(self, size=(1280, 720), fps=30):
        self.picam = Picamera2()
        # Format RGB888 → nanti konversi ke BGR untuk OpenCV
        config = self.picam.create_video_configuration(
            main={"size": size, "format": "RGB888"},
            controls={"FrameRate": fps}
        )
        self.picam.configure(config)
        # Opsi: kunci AE/AWB jika diperlukan kestabilan warna/exposure
    ...
    def read(self):
        # Dapatkan frame RGB lalu konversi ke BGR
        arr_rgb = self.picam.capture_array()
...
def main():
    ...
    # === Gunakan rpicam (Picamera2) ===
    if USE_PICAM:
        cap = PiCamCapture(size=(1280, 720), fps=30)
    else:
        # fallback (jika Picamera2 tidak tersedia)
    ...
    if not cap.isOpened():
        print("❌ Kamera tidak dapat diakses! Pastikan rpicam & python3-picamera2 terpasang.")
        return

    ret, frame = cap.read()
    if not ret or frame is None:
        print("❌ Gagal membaca frame pertama dari kamera.")
        cap.release()
        return
